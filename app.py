...
def addBookRoute():
    if request.method == "POST":
        ...
        response = addBook(mysql, bookID, title, genre, fname, lname, year, price, country, stock, image_url)
...
# Route to show the update form (GET)
@app.route("/updateBook", methods=["GET"])
def updateBookForm():
    bookID = request.args.get("bookID")
    bookData = bookDetail(mysql, bookID)
    return render_template("updatebook.html", bookData=bookData)
...
def updateBookRoute():
    ...
    response = updateBook(mysql, bookID, title, price, fname, lname, image_url, stock)
